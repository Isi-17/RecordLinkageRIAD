...
from src.data.preprocessor import DefaultBenchmarkPreprocessor
...
class DataincDataset(ABC):
    ...
    def create_instance(name: str,
        ...
        return DittoBenchmarkDataset(name=name, model_name=model_name, split_method=SplitMethod.PRE_SPLIT, seed=seed,
    ...
    def get_tokenized_data(self):
        try:
            ...
            if file_exists_or_create(tokenized_file_path):
                ...
                if 'lid' in self.tokenized_data.columns:
                    self.tokenized_data.set_index('lid', inplace=True)
                
                # 2. IMPORTANTE: Asegurar que el índice sea numérico.
                # El JSON suele cargar el índice como Strings ("0", "1"), 
                # pero el entrenamiento pide Integers (0, 1).
                self.tokenized_data.index = self.tokenized_data.index.astype(int)
        ...
        try:
            ...
            if file_exists_or_create(processed_matches_path):
                ...
                try:
                    ...
                    # Si el archivo está vacío, devolvemos un DataFrame vacío
...
class DittoBenchmarkDataset(DataincDataset):
    ...
    def __create_entity_data(self, filepath, dataset=1):
        def split_to_cols(input):
            ...
            return re.split(r"COL{1} {1}\w* VAL{1} {1}", input)

        def get_col_names(input):
            # return re.findall("(?<!COL)(\w+)(?=\s+VAL)", input)
            return re.findall(r"(?<!COL)(\w+)(?=\s+VAL)", input)
    ...
    def __create_df_from_txt_file(self, filepath) -> pd.DataFrame:
        def remove_formatting(txt_input):
            ...
            return re.sub(r"COL{1} {1}\w* VAL{1} {1}", '', txt_input).strip()
...
class PytorchDataset(Dataset):
    def __init__(self, model_name: str, idx_df: pd.DataFrame, data_df: pd.DataFrame, tokenizer: DataIncTokenizer,
        ...
        if 'lid' in self.data_df.columns:
            self.data_df.set_index('lid', inplace=True)
