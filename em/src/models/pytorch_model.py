...
class PyTorchModel:
        ...
        for epoch in tqdm(range(1, self.args.num_epochs + 1), desc=f'Training for {self.args.num_epochs} epochs ...'):
            ...
            for step, batch_tuple in tqdm(enumerate(self.train_data_loader),
                ...
                if self.args.use_softmax_layer:
                    # loss_fn = nn.NLLLoss()
                    weight = torch.tensor([1.0, self.args.pos_weight]).to(self.device)
            ...
            print(f"[Epoch {epoch}/{self.args.num_epochs}]\tTrain Loss: {train_loss}\tTrain Accuracy: {train_acc}")
            # logging.info(
    ...
    def test(self, epoch: int = 0, global_step: int = 0):
        ...
        print(f"[Epoch {epoch}/{self.args.num_epochs}]\tTest Loss: {test_loss}\tTest Accuracy: {test_acc}")
    ...
    def validate(self, epoch: int = 0, global_step: int = 0):
        ...
        for step, batch_tuple in tqdm(enumerate(self.val_data_loader), desc=f'[VALIDATE] Running epoch {epoch} ...',
            ...
            # Accumulate predictions and labels for ALL batches inside the loop
        ...
        print(f"[Epoch {epoch}/{self.args.num_epochs}]\tVal Loss: {val_loss}\tVal Accuracy: {val_acc}")
        print(f"Train F1: {f1:.4f} | Precision: {precision:.4f} | Recall: {recall:.4f}")
    ...
    def _reset_prediction_buffer(self, is_test=True):
        ...
        self.prediction_buffer = {
            'lids': np.zeros(num_test_samples, dtype=int),
            'rids': np.zeros(num_test_samples, dtype=int),
